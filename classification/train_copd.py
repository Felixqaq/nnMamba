...
def train_copd_model():
    ...
    # 創建 LoaderHelper 並設定為 Abnormal 任務
    ld_helper = LoaderHelper(task=Task.Normal_v_Abnormal)

    # ========== 繼續訓練設定 ==========
    resume_training = False  # 設為 True 繼續訓練
    start_fold = 0  # 從第 5 折開始（索引從 0 開始，所以 4 = 第5折）
    existing_uuid = "nnMamba_2026-01-05_16:11:17"  # 替換成你的實際 UUID
    # =================================
    ...
    model_uuid = train_camull(
        ...
        start_fold=start_fold if resume_training else 0,
        uuid=existing_uuid if resume_training else None,
    )

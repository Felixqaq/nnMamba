...
def main() -> None:
    ...
    parser.add_argument(
        "--title",
        default="MambaAngleRegressor",
        help="Figure title written into the SVG.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output SVG path. Defaults to docs/assets/mamba_regressor_architecture_auto.svg",
    )
    ...
    output_path = (
        ...
        if args.output is not None
            ...
            Path("./docs/assets/mamba_regressor_architecture_auto.svg"),
            config_dir,
        )
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(_svg_template(info, args.title), encoding="utf-8")
